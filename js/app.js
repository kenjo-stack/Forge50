...
const App={
 ...
 exerciseCard(s,e){
  const last=Store.last(e.id,e.weightMode,s.id),recommend=Store.recommend(e,s.id);const guide=!!ExerciseGuides.resolve(e.id).guide;const mode=e.weightMode==='per-dumbbell'?'kg per dumbbell':e.weightMode==='bodyweight'?'added kg (0 = bodyweight)':'kg total / machine setting';
  return `<section class="card exercise-card ${e.skipped?'skipped':''}" data-exercise="${e.id}"><div class="row"><h2>${this.esc(e.name)}</h2>${guide?this.button('Guide','guide',`data-name="${this.esc(e.id)}"`,'small-btn'):''}</div><p class="muted">${this.esc(e.muscle)} · ${this.esc(e.reps)} reps · target RIR ${e.rir} · ${Timer.format(e.restSeconds)} rest</p>
  ...
  <div class="row wrap space">${this.button('+ Set','add-set',`data-exercise="${e.id}"`,'small-btn')}${this.button('Remove last unlogged set','remove-set',`data-exercise="${e.id}"`,'small-btn')}${this.button('Rest · '+Timer.format(e.restSeconds),'rest',`data-seconds="${e.restSeconds}" data-name="${this.esc(e.id)}"`,'small-btn')}${this.button(e.skipped?'Include exercise':'Skip remaining sets','skip-exercise',`data-exercise="${e.id}"`,'small-btn')}</div>
